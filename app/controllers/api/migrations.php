...
use Getapp\Auth\OAuth2\Firebase as OAuth2Firebase;
use Getapp\Event\Event;
use Getapp\Event\Migration;
use Getapp\Extend\Exception;
use Getapp\Permission;
use Getapp\Role;
use Getapp\Utopia\Database\Validator\Queries\Migrations;
...
App::get('/v1/migrations/appwrite/report')
    ...
    ->label('sdk.description', '/docs/references/migrations/migration-getapp-report.md')
    ->label('sdk.response.code', Response::STATUS_CODE_OK)
    ->label('sdk.response.type', Response::CONTENT_TYPE_JSON)
    ...
    ->action(function (array $resources, string $endpoint, string $projectID, string $key, Response $response) {
        ...
        try {
            $report = $getapp->report($resources);
        } catch (\Throwable $e) {
            switch ($e->getCode()) {
                case 401:
                    throw new Exception(Exception::GENERAL_UNAUTHORIZED_SCOPE, 'Source Error: ' . $e->getMessage());
                case 429:
                    throw new Exception(Exception::GENERAL_RATE_LIMIT_EXCEEDED, 'Source Error: Rate Limit Exceeded, Is your Cloud Provider blocking Getapp\'s IP?');
                case 500:
...
App::get('/v1/migrations/firebase/deauthorize')
    ->desc('Revoke Getapp\'s authorization to access Firebase Projects')
    ->groups(['api', 'migrations'])
    ->label('scope', 'migrations.write')
    ->label('sdk.auth', [APP_AUTH_TYPE_ADMIN])
    ->label('sdk.namespace', 'migrations')
