...
use Getapp\Event\Build;
use Getapp\Event\Delete;
use Getapp\Event\Event;
use Getapp\Event\Func;
use Getapp\Event\Usage;
use Getapp\Event\Validator\FunctionEvent;
use Getapp\Extend\Exception;
use Getapp\Extend\Exception as AppwriteException;
use Getapp\Messaging\Adapter\Realtime;
use Getapp\Task\Validator\Cron;
use Getapp\Utopia\Database\Validator\CustomId;
use Getapp\Utopia\Database\Validator\Queries\Deployments;
use Getapp\Utopia\Database\Validator\Queries\Executions;
use Getapp\Utopia\Database\Validator\Queries\Functions;
...
App::post('/v1/functions/:functionId/deployments')
    ...
    ->action(function (string $functionId, ?string $entrypoint, ?string $commands, mixed $code, bool $activate, Request $request, Response $response, Database $dbForProject, Event $queueForEvents, Document $project, Device $deviceForFunctions, Device $deviceForLocal, Build $queueForBuilds) {
        ...
        if (!empty($contentRange)) {
            ...
            $deploymentId = $request->getHeader('x-getapp-id', $deploymentId);
...
App::post('/v1/functions/:functionId/executions')
    ...
    ->action(function (string $functionId, string $body, bool $async, string $path, string $method, array $headers, Response $response, Document $project, Database $dbForProject, Document $user, Event $queueForEvents, Usage $queueForUsage, string $mode, Func $queueForFunctions, Reader $geodb) {
        ...
        $headers['x-getapp-user-id'] = $user->getId() ?? '';
        $headers['x-getapp-user-jwt'] = $jwt ?? '';
