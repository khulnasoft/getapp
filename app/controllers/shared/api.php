...
$parseLabel = function (string $label, array $responsePayload, array $requestParams, Document $user) {
    ...
    foreach ($matches[1] ?? [] as $pos => $match) {
        ...
        if (count($parts) !== 2) {
            throw new Exception(Exception::GENERAL_SERVER_ERROR, "The server encountered an error while parsing the label: $label. Please create an issue on GitHub to allow us to investigate further https://github.com/khulnasoft/getapp/issues/new/choose");
        }
...
App::init()
    ...
    ->action(function (App $utopia, Request $request, Database $dbForConsole, Document $project, Document $user, ?Document $session, array $servers, string $mode) {
        ...
        $authKey = $request->getHeader('x-getapp-key', '');
