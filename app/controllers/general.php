...
use Getapp\Auth\Auth;
use Getapp\Event\Certificate;
use Getapp\Event\Event;
use Getapp\Event\Usage;
use Getapp\Extend\Exception as AppwriteException;
use Getapp\Network\Validator\Origin;
use Getapp\Utopia\Request;
use Getapp\Utopia\Request\Filters\V16 as RequestV16;
use Getapp\Utopia\Request\Filters\V17 as RequestV17;
use Getapp\Utopia\Response;
use Getapp\Utopia\Response\Filters\V16 as ResponseV16;
use Getapp\Utopia\Response\Filters\V17 as ResponseV17;
use Getapp\Utopia\View;
...
function router(App $utopia, Database $dbForConsole, callable $getProjectDB, SwooleRequest $swooleRequest, Request $request, Response $response, Event $queueForEvents, Usage $queueForUsage, Reader $geodb)
{
    ...
    if ($type === 'function') {
        ...
        $headers['x-getapp-trigger'] = 'http';
        $headers['x-getapp-user-id'] = '';
        $headers['x-getapp-user-jwt'] = '';
        $headers['x-getapp-country-code'] = '';
        $headers['x-getapp-continent-code'] = '';
        $headers['x-getapp-continent-eu'] = 'false';
        ...
        if (!empty($ip)) {
            ...
            if ($record) {
                ...
                $headers['x-getapp-country-code'] = $record['country']['iso_code'] ?? '';
                $headers['x-getapp-continent-code'] = $record['continent']['code'] ?? '';
                $headers['x-getapp-continent-eu'] = (\in_array($record['country']['iso_code'], $eu)) ? 'true' : 'false';
            }
        }
        ...
        if ($version === 'v2') {
            $vars = \array_merge($vars, [
                'APPWRITE_FUNCTION_TRIGGER' => $headers['x-getapp-trigger'] ?? '',
                'APPWRITE_FUNCTION_DATA' => $body ?? '',
                'APPWRITE_FUNCTION_USER_ID' => $headers['x-getapp-user-id'] ?? '',
                'APPWRITE_FUNCTION_JWT' => $headers['x-getapp-user-jwt'] ?? ''
            ]);
        }
...
App::init()
    ...
    ->action(function (Document $project, Request $request) {
        if ($project->getId() === 'console') {
            $message = empty($request->getHeader('x-getapp-project')) ?
...
App::init()
    ...
    ->action(function (App $utopia, SwooleRequest $swooleRequest, Request $request, Response $response, Document $console, Document $project, Database $dbForConsole, callable $getProjectDB, Locale $locale, array $localeCodes, array $clients, Reader $geodb, Usage $queueForUsage, Event $queueForEvents, Certificate $queueForCertificates) {
        ...
        */
        ...
        $requestFormat = $request->getHeader('x-getapp-response-format', System::getEnv('_APP_SYSTEM_RESPONSE_FORMAT', ''));
        if ($requestFormat) {
        ...
        $localeParam = (string) $request->getParam('locale', $request->getHeader('x-getapp-locale', ''));
        ...
        * Response format
        */
        $responseFormat = $request->getHeader('x-getapp-response-format', System::getEnv('_APP_SYSTEM_RESPONSE_FORMAT', ''));
        if ($responseFormat) {
        ...
        */
        ...
        if (
            ...
            && empty($request->getHeader('x-getapp-key', ''))
        ) {
...
App::error()
    ...
    ->action(function (Throwable $error, App $utopia, Request $request, Response $response, Document $project, ?Logger $logger, Log $log, Usage $queueForUsage) {
        ...
        if ($logger && $publish) {
            ...
            $log->addTag('locale', (string)$request->getParam('locale', $request->getHeader('x-getapp-locale', '')));
        ...
        /** Wrap all exceptions inside Getapp\Extend\Exception */
        if (!($error instanceof AppwriteException)) {
            $error = new AppwriteException(AppwriteException::GENERAL_UNKNOWN, $message, $code, $error);
        }
