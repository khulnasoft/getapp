...
use Getapp\Event\Audit;
use Getapp\Event\Build;
use Getapp\Event\Certificate;
use Getapp\Event\Database as EventDatabase;
use Getapp\Event\Delete;
use Getapp\Event\Event;
use Getapp\Event\Func;
use Getapp\Event\Mail;
use Getapp\Event\Messaging;
use Getapp\Event\Migration;
use Getapp\Event\Usage;
use Getapp\Extend\Exception;
use Getapp\GraphQL\Promises\Adapter\Swoole;
use Getapp\GraphQL\Schema;
use Getapp\Hooks\Hooks;
use Getapp\Network\Validator\Email;
use Getapp\Network\Validator\Origin;
use Getapp\OpenSSL\OpenSSL;
use Getapp\URL\URL as AppwriteURL;
...
App::setResource('user', function ($mode, $project, $console, $request, $response, $dbForProject, $dbForConsole) {
    /** @var Getapp\Utopia\Request $request */
    /** @var Getapp\Utopia\Response $response */
    ...
    if (empty($session['id']) && empty($session['secret'])) {
        $sessionHeader = $request->getHeader('x-getapp-session', '');
    ...
    $authJWT = $request->getHeader('x-getapp-jwt', '');
...
App::setResource('project', function ($dbForConsole, $request, $console) {
    ...
    $projectId = $request->getParam('project', $request->getHeader('x-getapp-project', ''));
...
App::setResource('mode', function ($request) {
     ...
     */
    return $request->getParam('mode', $request->getHeader('x-getapp-mode', APP_MODE_DEFAULT));
}, ['request']);
...
App::setResource('requestTimestamp', function ($request) {
    ...
    $timestampHeader = $request->getHeader('x-getapp-timestamp');
