...
Before making any changes, you will need to fork Appwrite's repository to keep branches on the official repo clean. To do that, visit the [Appwrite Github repository](https://github.com/khulnasoft/getapp) and click on the fork button.

![Fork button](images/fork.png)

This will redirect you from `github.com/khulnasoft/getapp` to `github.com/YOUR_USERNAME/appwrite`, meaning all changes you do are only done inside your repository. Once you are there, click the highlighted `Code` button, copy the URL and clone the repository to your computer using `git clone` command:
...
We maintain a [`locale branch`](https://github.com/khulnasoft/getapp/tree/locale/) under the [khulnasoft/getapp repo](https://github.com/khulnasoft/getapp/) exclusively for translations related PRs. Here are a few files that you need to refer to help with your contribution.

1. **terms.json**

   [terms.json](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/terms.json) contains all the terms that are used in Appwrite that require translation. Each term is a JSON object as shown below.
...
2. **en.json**
   ...
   [en.json](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/translations/en.json) contains the English translation for all the terms that are present in **terms.json**. You can use this file as a reference when making a contribution for your language.
...
3. **languages.php**
   ...
   [languages.php](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/languages.php) contains all the languages listed in **[ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)**. You can use this file to find your language code when making a contribution for your language.

Great, let's start. First, find the code of the language you want to add. For example, if you want to add support for **Spanish**, you can find the code for Spanish in [languages.php](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/languages.php#L202).

Once you have found the ISO language code for **Spanish** (es), create a new file `/app/config/locale/translations/es.json` just like all [the other languages](https://github.com/khulnasoft/getapp/tree/locale/app/config/locale/translations).

Next, choose a reference language. If English is your reference language, copy the contents of [`en.json`](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/translations/en.json) into `/app/config/locale/translations/es.json` and translate all the corresponding strings like so
...
After exporting a JSON file, we need to rename it to follow the **[ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes)** standards. You can use [languages.php](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/languages.php) file to find your language code when making a contribution for your language. For example, Spanish translation should have a file called `es.json`.

## 3. Add the translations to the project

Add your language code to [codes.php](https://github.com/khulnasoft/getapp/blob/locale/app/config/locale/codes.php#L14) in the following format.
...
Finally, load your translation file in `init.php` by following a pattern similar to the [existing languages](https://github.com/khulnasoft/getapp/blob/locale/app/init.php#L270).
...
If you are lost in the Appwrite dashboard, check out our [Article about Appwrite's dashboard](https://dev.to/appwrite/30daysofgetapp-getapp-dashboard-15cc).
